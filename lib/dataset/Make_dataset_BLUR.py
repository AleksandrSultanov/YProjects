...
for row in total_data.iterrows():
    ...
    cv2.imwrite("Новая папка/" + row[1].user_id + "/landmark_aligned_face." + str(row[1].face_id) + ".BLUR_" + row[
        1].original_image, output)

    frame = pd.DataFrame([[row[1].user_id, "BLUR_" + row[1].original_image, row[1].face_id, row[1].age, row[1].gender,
                           row[1].x, row[1].y,
